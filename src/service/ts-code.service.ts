...
import {I${_.upperFirst(_.camelCase(sheet.name))}GeneratorNodeData} from "./${sheet.name}.interface.generated";
import {I${_.upperFirst(_.camelCase(sheet.name))}GeneratorNodeExport} from "./${sheet.name}.interface.generated";
...
`);
    super.save(sheet.name + ".interface", source);
  }
  ...
  saveAll(): boolean {
    ...
    _.each(_.difference(this.list(), _.chain(this.$hub.sheets).keys().flatMap(name => [name, `${name}.interface`]).value()), name => this.unlink(name));
    return true;
  }
