  ...
  ul.list-group
    ...
    li.list-group-item(v-if="column")
      div.row
        ...
        div.col-xs-6
          button.btn.btn-block.btn-warning(@click="move(true)", :disabled="columnIndex == currentSheet.columns.length - 1") Right #[i.fa.fa-arrow-right]
    li.list-group-item(v-if="column")
      div.row
        div.col-xs-12
    ...
    li.list-group-item(v-if="column")
      div.row
        div.col-xs-12
          button.btn.btn-block.btn-primary(@click="freeze()", :class="{'active': columnIndex + 1 == currentSheet.freezeColumn}") #[i.fa.fa-map-pin] Freeze Column
